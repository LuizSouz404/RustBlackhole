...
export function MenuItem({ hotkey = '', children, className, ...props }: MenuItemProps) {
  ...
  return (
    <button 
      ref={buttonRef}
      className={twMerge("flex group px-2.5 py-1.5 cursor-default items-center rounded-sm gap-2 w-full enabled:hover:bg-blue-500/25 disabled:opacity-40 aria-pressed:bg-blue-400/25", className)}
      {...props}
    >
      {children}
